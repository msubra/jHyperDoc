...
function InitMiate(){
	if(IniFlg){	InitM(FrstCntnr);IniFlg=0;
		if(ShwFlg)AfterCloseAll();ShwFlg=0}}

function InitM(CntnrPntr){
	...
	while(Mmbr!=null){
		...
		if(Mmbr.ChildCntnr) InitM(Mmbr.ChildCntnr);
		Mmbr=Mmbr.PrvMbr}
	RcrsLvl--}
...
function CloseMenu(){
	...
	if(this==CurrntOvr){
		IniFlg=1;
		if(CloseTmr)clearTimeout(CloseTmr);
		CloseTmr=setTimeout('InitMiate(CurrntOvr)',DissapearDelay)}}
